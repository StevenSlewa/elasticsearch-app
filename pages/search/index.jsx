            ...
            if (index !== -1) {
                ...
                return (
                    <span>
                        ...
                        {fragment.split(RegExp(`(${searchValue})`, "gi"))
                            .map((part, i) => {
            ...
            <section className="max-w-xl mx-auto my-8">
                <div className="relative w-full max-w-md mx-auto">
                    <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                        <SearchIcon className={`w-6 h-6 text-white transform  duration-500 ease-in-out transition-transform  ${isLoading ? 'animate-pulse rotate-45 -translate-x-2 ' : ''}`} />
                    </div>
                    <input
                    ...
                    files.map((file) => {
                        return (
                            <div key={file._id} onClick={() => { handleSave(file.title, file._id); }} className="w-full mb-4 text-gray-400 rounded-md cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600" >
                                ...
                                <div className="p-4">
                                    {searchValue.trim() != '' ? getHighlightedContent(file) : ''}
                                </div>
                            </div>
                        );
                    })
                                            ...
                                            <span>
                                                {
                                                    ...
                                                    file.attachment?.content?.split(RegExp(`(${searchValue})`, "gi"))
                                                    .map((part, i) => {
